...
class DataTransformation:
    ...
    def get_data_tranformer_object(cls)->Pipeline:
        """This function returns the sklearn pipeline object"""
        try:
    ...
    def initiate_data_transforamtion(self)->DataTransformationArtifact:
        try:
            ...
            #dropping target or output column from trainining dataset
            input_feature_train_df = train_df.drop(columns=[TARGET_COLUMN], axis=1)
            target_feature_train_df = train_df[TARGET_COLUMN]
            #mapping target values to postitive and negative in train dataset
            target_feature_train_df = target_feature_train_df.replace(TargetValueMapping().to_dict())
            #dropping target or output columns on testting dataset
            ...
            #now concatinationg the two datasets
            train_arr = np.c_[input_feature_train_final,np.array(target_feature_train_final)]
            test_arr = np.c_[input_feature_test_final,np.array(target_feature_test_final)]            
            
            #save train and test array
            save_numpy_array_data(self.data_transforamtion_config.transformed_train_file_path,array=train_arr)
            save_numpy_array_data(self.data_transforamtion_config.transformed_test_file_path,array=test_arr)
            
            #save sklearn pipeline object
            save_object(self.data_transforamtion_config.transformed_object_file_path,preprocessor_object)
            #preparing artifact
